...
\`\`\`
{
  "exercises": [
    {
      "name": string,           // Полное название упражнения
      ...
      "weight": string,         // Рекомендуемый вес с единицей измерения
      "cues": string,           // Технические подсказки для выполнения
      "targetMuscles": string[] // Список задействованных мышц
    }
    // ... повторить для каждого упражнения
  ],
  "adaptationNotes": string[], // Опционально: заметки об адаптации
  "warnings": string[]         // Опционально: предупреждения
}
\`\`\`

**ВАЖНО:** Каждое упражнение ДОЛЖНО содержать поле "rest" (число в секундах)!
