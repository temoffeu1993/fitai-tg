...
const ABSOLUTE_MAX_EXERCISES = 20;
const ABSOLUTE_MIN_EXERCISES = 2;
...
const DEFAULT_EXERCISES_COUNT = 7;
...
const prompt = `Ты персональный тренер. Определи разумное количество упражнений для одной тренировки.

Учитывай:
- профиль (пол, возраст, уровень, цели, дни в неделю, длительность сессии),
- чек-ин (сон, стресс, энергия, мотивация, боли, доступное время),
- историю тренировок (RPE, частота, примерный объём),
- то, как реальный тренер подберёт объём для такого человека именно сегодня.

Твоя задача:
- предложить такой объём, который не выглядит шаблонным,
- при плохом самочувствии и высоком стрессе объём может быть меньше,
- при хорошем самочувствии и опыте — объём может быть больше,
- не обязан во что бы то ни стало «забивать» всё время до минуты.

Верни JSON:
{
  "count": число_упражнений,
  "reason": "кратко объясни, почему такой объём"
}
...
Это первая тренировка клиента. История тренировок отсутствует.`);
  } else {
...
История тренировок в системе пока отсутствует.
Неделя программы (от первой зафиксированной тренировки): ${week}.`;
  }
  ...
  const avgRpe = rpes.length > 0 ? Number((rpes.reduce((a, b) => a + b, 0) / rpes.length).toFixed(1)) : null;

  let volumeLine = "- Данные по объёму пока недостаточны.";
  if (volumes.length >= 1) {
    volumeLine = `- Средний объём за последние ${volumes.length} тренировок: ~${avgVolume} кг`;
  }

  let rpeLine = "- Данные по RPE пока недостаточны.";
  if (avgRpe != null && rpes.length >= 1) {
    rpeLine = `- Средний RPE за последние ${rpes.length} тренировок: ${avgRpe}/10`;
  }
...
function validatePlanStructure(
  ...
  if (normalized.exercises.length < ABSOLUTE_MIN_EXERCISES && sessionMinutes >= 60) {
    warnings.push(
      `AI создал только ${normalized.exercises.length} упражнение — для длинной сессии это очень мало`
    );
  }
  ...
  normalized.exercises = normalized.exercises.map((ex) => {
    ...
    if (!Number.isFinite(sets) || sets <= 0) {
      ...
      sets = 15;
      warnings.push(`${ex.name}: слишком много подходов (${ex.sets}), ограничили до 15`);
    }
    ...
    if (!updated.reps || (typeof updated.reps === "string" && !/\d/.test(updated.reps))) {
      updated.reps = "6–15";
      warnings.push(`${ex.name}: не указаны повторы, поставили 6–15`);
    }
    ...
    if (!Number.isFinite(restSec) || restSec <= 0) {
      ...
      restSec = 10;
      warnings.push(`${ex.name}: отдых ${ex.restSec} сек слишком мал, подняли до 10`);
    } else if (restSec > 600) {
      restSec = 600;
      warnings.push(`${ex.name}: отдых ${ex.restSec} сек слишком велик, ограничили 600`);
    }
