...
export function filterExercisesForPattern(args) {
    ...
    const filtered = EXERCISE_LIBRARY.filter(ex => {
        // Pattern match
        if (!ex.patterns.includes(pattern))
            return false;
        // Experience gate
        if (lvRank(ex.minLevel) > lvRank(constraints.experience))
            return false;
        // Equipment gate
        ...
        if (constraints.equipmentAvailable?.length) {
            ...
            const hasGymFull = constraints.equipmentAvailable.some((eq) => eq === "gym_full");
            if (!hasGymFull) {
                // Проверяем что ВСЁ оборудование упражнения доступно
                const allEquipmentAvailable = ex.equipment.every(eq => constraints.equipmentAvailable.includes(eq) || eq === "bodyweight");
        ...
        if (constraints.avoid?.length && ex.jointFlags?.length) {
            if (ex.jointFlags.some(f => constraints.avoid.includes(f)))
                return false;
        }
        return true;
    });
...
export function scoreExerciseAdvanced(args) {
    ...
    if (ctx.timeBucket === 45) {
        ...
        if (hasAnyTag(ex, ["not_for_circuit"]))
            s -= 4;
    }
    ...
    if (ctx.intent === "hard") {
        ...
        s += ex.difficulty * 2.0;
        if (hasAnyTag(ex, ["strength_bias", "barbell_skill"]))
            s += 3;
    }
    // -------------------------------------------------------------------------
    ...
    if (role === "main") {
        ...
        if (ex.kind === "isolation")
            s -= 5;
        if (hasAnyTag(ex, ["strength_bias", "progression_ready"]))
            s += 8;
        if (ex.repRangeDefault.min <= 6)
            s += 3;
    }
    if (role === "secondary") {
    ...
    if (role === "accessory") {
        ...
        if (ex.kind === "compound")
            s -= 3;
        if (hasAnyTag(ex, ["constant_tension", "hypertrophy_bias"]))
            s += 5;
        if (ex.repRangeDefault.min >= 10)
            s += 3;
    }
    if (role === "pump") {
        ...
        if (ex.setupCost <= 2)
            s += 5;
        if (hasAnyTag(ex, ["constant_tension", "good_for_circuit"]))
            s += 6;
    }
    if (role === "conditioning") {
        // Conditioning: full body, circuits, low setup
        if (hasAnyTag(ex, ["full_body_cardio", "good_for_circuit"]))
            s += 12;
        if (ex.setupCost >= 4)
            s -= 8;
    }
    ...
    if (ctx.goal === "build_muscle") {
        if (hasAnyTag(ex, ["hypertrophy_bias", "constant_tension"]))
            s += 7;
        if (ex.kind === "isolation" && role !== "main")
            s += 3;
    }
    if (ctx.goal === "lose_weight") {
        if (hasAnyTag(ex, ["good_for_circuit", "fat_loss_friendly", "full_body_cardio"]))
            s += 10;
        if (hasAnyTag(ex, ["not_for_circuit"]))
            s -= 8;
    ...
    if (ctx.goal === "athletic_body" || ctx.goal === "health_wellness") {
        if (hasAnyTag(ex, ["good_for_circuit", "low_setup", "functional"]))
            s += 5;
        if (ex.unilateral)
            s += 3;
    }
    if (ctx.goal === "lower_body_focus") {
        ...
        if (["squat", "hinge", "lunge", "hip_thrust"].includes(pattern)) {
            s += 5;
            if (ex.primaryMuscles.some(m => ["glutes", "quads", "hamstrings"].includes(m)))
                s += 8;
        }
    }
    if (ctx.preferCircuits) {
        if (hasAnyTag(ex, ["good_for_circuit"]))
            s += 6;
        if (ex.setupCost >= 4)
            s -= 6;
    }
    ...
    if (ex.unilateral && ctx.intent !== "hard")
        s += 2;
    // Mixed planes for athletic
    if (ctx.goal === "athletic_body" && ex.plane === "mixed")
        s += 2;
    // Progression-ready bonus
    if (hasAnyTag(ex, ["progression_ready", "easy_to_progress"]))
        s += 2;
    // Slight bonus for safe choices
    if (ctx.intent === "light" && hasAnyTag(ex, ["stable_choice"]))
        s += 2;
    // -------------------------------------------------------------------------
    ...
    if (ctx.intent === "light") {
        if (ex.cnsLoad === 1 || hasAnyTag(ex, ["low_cns"]))
            s += 6;
        if (ex.cnsLoad === 3 || hasAnyTag(ex, ["high_cns"]))
            s -= 5; // Было -10, стало -5 - мягче
    }
...
export function pickExercisesForPattern(args) {
    const { pattern, n, role, ctx, constraints, excludeIds = [], } = args;
    const usedIds = args.usedIds ?? new Set();
    const usedPatterns = args.usedPatterns ?? new Map();
    const usedPrimaryMuscles = args.usedPrimaryMuscles ?? new Map();
    ...
    if (!pool.length && args.isRequiredPattern) {
        ...
        const hasGymFull = eqAvail.includes("gym_full");
        pool = EXERCISE_LIBRARY.filter(ex => {
            ...
            if (constraints.avoid?.length) {
                ...
                if (ex.jointFlags?.some(jf => avoidFlags.includes(jf)))
                    return false;
            }
            return true;
        });
    ...
    if (args.isRequiredPattern) {
        ...
        if (dupPool.length) {
            ...
            return pickExercisesFromPool(dupPool, n, { ...normArgs, usedIds: new Set() }, true);
        }
    }
    return selected;
}
// Helper function to avoid duplication
function pickExercisesFromPool(pool, n, args, isRequiredPattern) {
...
export function selectExercisesForDay(args) {
    const usedIds = new Set(args.excludeIds ?? []);
    const usedPatterns = new Map();
    const usedPrimaryMuscles = new Map();
    ...
    for (const slot of expandedSlots) {
        if (!grouped.has(slot.pattern))
            grouped.set(slot.pattern, []);
    ...
    const requiredPicked = new Set();
    for (const rp of required) {
        const picked = pickExercisesForPattern({
            pattern: rp,
            n: 1,
            role: "main", // Required patterns deserve main role priority
            ctx: args.ctx,
        ...
        if (picked.length) {
            out.push({ ex: picked[0], pattern: rp, role: "main" });
...
// ============================================================================
export { filterExercisesForPattern as filterExercisesForSlot, scoreExerciseAdvanced as scoreExercise, pickExercisesForPattern as pickExercises, };
