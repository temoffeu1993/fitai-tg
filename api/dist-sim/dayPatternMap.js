...
function assignRole(pattern, isFirst, isDouble) {
...
export function buildDaySlots(args) {
    ...
    if (availableMinutes) {
        if (timeBucket === 45 && availableMinutes < 40)
            slotBudget = range.min;
        if (timeBucket === 45 && availableMinutes > 50)
            slotBudget = range.max;
        if (timeBucket === 60 && availableMinutes < 50)
            slotBudget = range.min;
        if (timeBucket === 60 && availableMinutes > 70)
            slotBudget = range.max;
        if (timeBucket === 90 && availableMinutes < 75)
            slotBudget = range.min;
        if (timeBucket === 90 && availableMinutes > 95)
            slotBudget = range.max;
    }
    const slots = [];
    ...
    for (const pattern of rules.required) {
        if (usedBudget >= slotBudget)
            break; // ИСПРАВЛЕНО: usedBudget вместо slots.length
        const canDouble = !!rules.preferredDoubles?.includes(pattern) && (usedBudget <= slotBudget - 2); // ИСПРАВЛЕНО: проверяем достаточно ли места для double
    ...
    if (usedBudget < slotBudget) {
        ...
        if (templateRulesId.toLowerCase().includes("push")) {
            if (!usedPatterns.has("rear_delts"))
                fillerPatterns.push("rear_delts");
            if (!usedPatterns.has("core"))
                fillerPatterns.push("core");
            if (!usedPatterns.has("calves"))
                fillerPatterns.push("calves");
        }
        ...
        if (templateRulesId.toLowerCase().includes("upper")) {
            ...
            if (!usedPatterns.has("calves"))
                fillerPatterns.push("calves");
            if (!usedPatterns.has("biceps_iso"))
                fillerPatterns.push("biceps_iso");
            if (!usedPatterns.has("triceps_iso"))
                fillerPatterns.push("triceps_iso");
            if (!usedPatterns.has("rear_delts"))
                fillerPatterns.push("rear_delts");
        }
        ...
        while (usedBudget < slotBudget && usedPatterns.size > 0) {
            ...
            const compoundPatterns = existingPatterns.filter(p => ["squat", "hinge", "lunge", "horizontal_push", "incline_push", "vertical_push",
