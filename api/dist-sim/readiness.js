...
export function computeReadiness(args) {
    ...
    if (checkin?.sleep === "poor") {
        warnings.push("😴 Плохой сон может снизить восстановление и координацию. " +
    ...
    // ЭНЕРГИЯ
    if (checkin?.energy === "low") {
        warnings.push("🔋 Низкая энергия. Сократи рабочие веса на 10-15% или уменьши количество подходов.");
    ...
    // СТРЕСС
    if (checkin?.stress === "very_high") {
        warnings.push("😰 Очень высокий стресс. Тренировка поможет, но избегай максимальных весов. " +
    ...
    if (checkin?.pain && checkin.pain.length > 0) {
        ...
        if (maxPainLevel >= 7) {
            warnings.push(`🔴 Сильная боль: ${painDesc}. Упражнения адаптированы, но если боль усиливается — останови тренировку.`);
        }
        else if (maxPainLevel >= 4) {
            warnings.push(`⚠️ Боль: ${painDesc}. Упражнения подобраны с учётом этого. Избегай дискомфорта, снижай веса при необходимости.`);
        }
        else {
    ...
    if (effectiveMinutes && effectiveMinutes < fallbackTimeBucket) {
        notes.push(`⏱️ Доступно ${effectiveMinutes} мин (обычно ${fallbackTimeBucket}). ` +
    ...
    if (checkin?.energy === "low" && checkin?.sleep === "poor") {
        notes.push("⚠️ Сочетание низкой энергии и плохого сна — сигнал организму. " +
    ...
    const corePolicy = (effectiveMinutes !== null && effectiveMinutes < 40) || timeBucket === 45
        ? "optional"
        : "required";
    // -------------------------------------------------------------------------
    // 9. RETURN READINESS
 ...
 */
export function normalizeBlockedPatterns(blocked) {
...
// ============================================================================
function mapPainToBlocks(location, level) {
    const flags = [];
    const patterns = [];
    const dayTypes = [];
