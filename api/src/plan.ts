...
// Правила фокуса дня (мягкое ограничение, чтобы не смешивать противоположные группы)
function buildFocusRules(todayFocus: string): string {
  return `## Соблюдай фокус дня: "${todayFocus}"

**Правило:** если в названии дня указан конкретный фокус (Push/Pull/Legs/Upper/Lower/Glutes и т.д.), включай ТОЛЬКО упражнения для этого фокуса:
- Push/Толкающие → грудь, плечи (передние/средние), трицепс. БЕЗ тяг, спины, бицепса, ног
- Pull/Тяговые → спина, задние дельты, бицепс, трапеции. БЕЗ жимов, груди, плеч, трицепса, ног
- Legs/Lower/Ноги → квадрицепс, ягодицы, бицепс бедра, икры. БЕЗ упражнений на верх тела
- Upper/Верх → жимы И тяги верхом сбалансированно. БЕЗ ног
- Full Body → сбалансируй все: ноги + жимы верхом + тяги верхом
- Специализация (Glutes/Плечи/и т.д.) → акцент на указанную группу, остальное минимум

Если день называется нестандартно — определи по контексту программы и истории, какой паттерн движений логичен для этого дня, и не смешивай противоположные группы.`;
}

// Мягкие подсказки по балансу мышц
function buildBalanceRules(): string {
  return `## Баланс нагрузки
- В Upper/Push основной объём на грудь/передние‑средние дельты/трицепс; тяги/спину давай минимально, без перекоса в спину.
- Избегай перегруза одной цепочки: не давай 3+ однотипных тяг/жимов подряд, чередуй углы и оборудование.
- В Legs держи разумный объём: 1–2 базовых + 2–3 изоляции, без избыточной осевой нагрузки подряд.
- В Full Body дай сбалансированно: ноги + толкающее верхом + тягующее верхом, без перекоса в одну группу.
- Разнообразие углов/инвентаря > дубликация одного паттерна подряд.`;
}
