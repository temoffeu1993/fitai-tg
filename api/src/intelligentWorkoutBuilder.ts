...
Для КАЖДОГО дня определи ИНДИВИДУАЛЬНУЮ структуру:
...
**КРИТИЧНО ВАЖНО:**
- **КАЖДЫЙ ДЕНЬ УНИКАЛЕН!** Анализируй фокус и описание КАЖДОГО дня отдельно!
- Разные группы мышц требуют разного объёма, количества упражнений и отдыха
- Учитывай анатомию и физиологию целевых групп мышц
- Размер мышц, количество суставов, энергозатраты — всё влияет на структуру
- Не используй одинаковую структуру для разных дней!

**На основе Volume Landmarks (MEV/MAV/MRV):**
- Какой недельный объём нужен для ЭТОЙ конкретной группы мышц?
- Как распределить этот объём учитывая частоту тренировок?
- Сколько упражнений и подходов оптимально для достижения MAV в ЭТОТ ДЕНЬ?

**Контекст:**
- Используй ВСЁ доступное время (${userProfile.timeAvailable} минут)
- Частота: ${daysRules.length} дней/неделю
- Уровень: ${userProfile.experience}
- Цель: ${userProfile.goal}

**ДОВЕРЯЮ твоей экспертизе! Создай ОПТИМАЛЬНУЮ и ИНДИВИДУАЛЬНУЮ структуру для каждого дня!**
...
Верни ТОЛЬКО JSON, без markdown!`;
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      temperature: 1.0, // Максимальная креативность для разнообразия структур
      response_format: { type: "json_object" }
    });
