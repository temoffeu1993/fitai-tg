...
export function getSessionCaps(experience, timeBucket, intent) {
...
// ============================================================================
export const GOAL_VOLUME_MULTIPLIER = {
...
export const DAYS_FREQUENCY_MODIFIER = {
...
export function calculateSetsForSlot(args) {
    ...
    if (process.env.DEBUG_VOLUME === "1" && sets > 6) {
        console.warn(`⚠️  Volume clamp: ${role}/${experience} wanted ${sets} sets ` +
...
export function getRepsRange(args) {
    ...
    if (intent === "light") {
        ...
        reps = [reps[0], Math.max(reps[0] + 2, reps[1] - 2)];
    }
    else if (intent === "hard") {
        // Increase upper bound slightly
        reps = [reps[0], Math.min(reps[1] + 2, 30)];
    }
    return reps;
}
// ============================================================================
...
export const REST_MODIFIERS_BY_EXPERIENCE = {
 ...
 */
function roundRestTime(seconds) {
...
export function validateWorkoutVolume(args) {
    ...
    if (overMaxSets) {
        warnings.push(`⚠️ Превышение maxSets: ${totalSets} > ${caps.maxSets} (${experience}, ${timeBucket}min, ${intent})`);
    }
    // Проверка количества упражнений
    if (overMaxExercises) {
        warnings.push(`⚠️ Слишком много упражнений: ${totalExercises} > ${caps.maxExercises}`);
    }
    if (underMinExercises) {
        warnings.push(`⚠️ Слишком мало упражнений: ${totalExercises} < ${caps.minExercises} (недоиспользование ${timeBucket}min)`);
    }
...
export function mapMuscleToVolumeGroup(muscle) {
    const map = {
        chest: "chest",
        lats: "back",
        upper_back: "back",
...
export function validateWeeklyMuscleVolume(args) {
    ...
    for (const [muscle, sets] of muscleVolumes) {
        if (sets > maxPerMuscle) {
            ...
            warnings.push(`⚠️ ${muscle}: ${sets} подходов/нед > ${maxPerMuscle} (лимит для ${experience})`);
        }
    }
