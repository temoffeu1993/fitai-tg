...
function buildSystemPrompt(): string {
  return [
    ...
    "- Не утверждай фактическое время отдыха между подходами: в данных есть только отдых «по плану» (restSec).",
    "",
    "Язык:",
...
function buildUserPrompt(args: { question: string; context: any; focus: any }): string {
  return [
    ...
    "   - Если конкретики нет — прямо скажи, чего нет в данных (например: «фактический отдых между подходами не логируется»).",
