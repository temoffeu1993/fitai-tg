...
async function callAIForWorkout(context: WorkoutGenerationContext): Promise<{
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      temperature: 0.8, // Больше уверенности в объёме
...
**МАКСИМАЛЬНО ИСПОЛЬЗУЙ доступное время (${userProfile.timeAvailable} минут)!**

Определи на основе своих знаний Volume Landmarks:
