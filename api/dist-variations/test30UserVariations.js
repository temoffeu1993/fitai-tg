...
import { generateWeekPlan, generateWorkoutDay, generateRecoverySession, } from "./workoutDayGenerator.js";
import { computeReadiness } from "./readiness.js";
import { decideStartAction } from "./checkinPolicy.js";
function mapGoalToNew(oldGoal) {
    const goalMap = {
...
function mapEquipmentToNew(location, equipmentList) {
    if (location === "gym" || equipmentList.includes("barbell") || equipmentList.includes("machines")) {
        return "gym_full";
    }
    if (equipmentList.includes("dumbbells"))
        return "dumbbells";
    if (equipmentList.includes("bands"))
        return "limited";
    return "bodyweight";
}
function calculateTimeBucket(minutes) {
    if (minutes <= 50)
        return 45;
    if (minutes <= 75)
        return 60;
...
function buildConstraintsFromAgeBmi(age, bmi) {
    const constraints = [];
    if (age >= 50) {
...
function requiredPatternsForDay(scheme, dayIndex) {
    return scheme.days[dayIndex]?.requiredPatterns ? [...scheme.days[dayIndex].requiredPatterns] : [];
}
function patternsInWorkoutDay(day) {
    const s = new Set();
    for (const ex of day.exercises ?? []) {
        for (const p of ex.exercise?.patterns ?? [])
            s.add(String(p));
    }
    return s;
}
function validateRequiredPatterns(args) {
...
function pickCheckinsForUser(i) {
    const alwaysOk = {
        name: "Чекин: норм/готовность",
        input: { energy: "medium", sleep: "good", stress: "low", pain: [], soreness: [] },
    };
    const variants = [
        {
...
function validateAdaptationAgainstReadiness(args) {
    ...
    for (const ex of args.day.exercises ?? []) {
        const flags = ex.exercise?.jointFlags ?? [];
...
function analyzeWeekBasics(args) {
    const issues = [];
    const warnings = [];
    ...
    if (pushSets > 0 && pullSets > 0) {
        ...
        if (ratio > 1.35)
            warnings.push(`Дисбаланс Push/Pull: push=${pushSets} pull=${pullSets} ratio=${ratio.toFixed(2)}`);
    }
    ...
    for (const day of args.weekPlan) {
        ...
        if ((day.totalExercises ?? 0) === 0)
            issues.push(`День "${day.dayLabel}" без упражнений`);
    }
    return { issues, warnings };
}
function buildOnboardingPool() {
...
function selectFirstNWithCandidates(pool, n) {
    const chosen = [];
    for (const v of pool) {
...
function main() {
    ...
    for (let i = 0; i < selected.length; i++) {
        ...
        for (const { name, input } of checkins) {
            ...
            const dayIndexUsed = decision.action === "swap_day" && "targetDayIndex" in decision ? decision.targetDayIndex : 0;
            const baselineDay = weekPlan[dayIndexUsed];
            let workoutDay = null;
            let workoutKind = "regular";
            ...
            const adaptation = validateAdaptationAgainstReadiness({
                day: workoutDay,
                readinessBlockedPatterns: readiness.blockedPatterns,
                readinessAvoidFlags: readiness.avoidFlags,
                availableMinutes: input.availableMinutes,
            });
            ...
            checkinResults.push({
                ...
                readiness: {
                    ...
                    blockedDayTypes: readiness.blockedDayTypes,
